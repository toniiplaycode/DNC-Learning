...
INSERT INTO `assignment_submissions` VALUES (1,3,15,'Đây là bài nộp cho bài tập thực hành về cấu trúc điều khiển trong C/C++','uploads/assignments/user15_assignment3.cpp','2025-04-06 11:22:16',NULL,NULL,'submitted','2025-04-06 11:22:16','2025-04-06 11:22:16'),(2,1,1,'Cho em nộp bài Python OOP','uploads/assignments/user1_assignment.py','2025-04-06 11:22:16',NULL,NULL,'submitted','2025-04-06 11:22:16','2025-04-06 11:22:16');
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `before_quiz_response_insert_calculate_score` BEFORE INSERT ON `quiz_responses` FOR EACH ROW BEGIN
  DECLARE is_correct_val TINYINT;
  
  -- Chỉ tính điểm khi có cả question_id và selected_option_id
  IF NEW.question_id IS NOT NULL AND NEW.selected_option_id IS NOT NULL THEN
    -- Lấy thông tin xem câu trả lời đúng không
    SELECT is_correct INTO is_correct_val
    FROM quiz_options
    WHERE id = NEW.selected_option_id;
    
    -- Gán điểm dựa trên kết quả
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `before_quiz_response_update_calculate_score` BEFORE UPDATE ON `quiz_responses` FOR EACH ROW BEGIN
  DECLARE is_correct_val TINYINT;
  
  -- Chỉ tính điểm khi có cả question_id và selected_option_id
  IF NEW.question_id IS NOT NULL AND NEW.selected_option_id IS NOT NULL AND 
     (OLD.selected_option_id <> NEW.selected_option_id OR NEW.score IS NULL) THEN
    -- Lấy thông tin xem câu trả lời đúng không
    SELECT is_correct INTO is_correct_val
    FROM quiz_options
    WHERE id = NEW.selected_option_id;
...
-- Dump completed on 2025-04-18 10:48:27
